...
def plot_e_salvar(Ns, means, stds_abs, title, xlabel, ylabel, output_path):
    ...
    # Plot principal com barras de erro
    plt.errorbar(
        Ns, means, 
        yerr=stds_abs, 
        fmt='o-',         # Marcadores circulares + linha contínua
        linewidth=2,      # Espessura da linha
        markersize=8,     # Tamanho dos marcadores
        capsize=5,        # Tamanho das hastes horizontais
        capthick=2,       # Espessura das hastes
        ecolor='#d62728', # Cor vermelha para as barras de erro
        elinewidth=2,     # Espessura das barras de erro
        label='Média e Desvio Padrão'
    )
    
    # Preenchimento entre (média - std) e (média + std)
    plt.fill_between(
        Ns, 
        means - stds_abs, 
        means + stds_abs,
    ...
    # Rótulos do eixo X apenas para os valores de N existentes
    plt.xticks(Ns, [str(n) for n in Ns], fontsize=12, rotation=45)
